...
export let BitBoxRoute = express.Router()

BitBoxRoute.get("/", async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            const element = user.alertBox.Bits
            Object.assign(obj, {
                [obj.boxes]: {
                    color: element.settings.color,
                    state: element.settings.state,
        ...
        res.render("main/boxes/bitBox.ejs", obj);
    } catch (error) {
        console.log(error);
        res.redirect("/bits");
    };
...
BitBoxRoute.get("/:key/renew", async (req, res) => {
    const key = req.signedCookies.access_validator;
    if (!key) {
        return res.redirect("/?index=true");
    };
...
BitBoxRoute.post('/save', async (req, res) => {
    ...
    try {
        ...
        if (user) {
            const alertBox = user.alertBox.Bits.settings;
        ...
        await Insert.UpdateAlertBoxSettings([sessionData.userId, "Bits", JSON.stringify({ state, volume, duration, responseText, color, layout: selectedLayout, viewer, streamer, amount, family, size, decoration, weight })]);
    } catch (error) {
...
BitBoxRoute.post('/upload/bit',
    ...
    async (req, res) => {
        ...
        try {
            ...
            const userFolder = path.join("uploads", "bits", String(userId));
            ...
            if (req.files && req.files['image']) {
                ...
                await Insert.UpdateAlertBoxImage([userId, "Bits", imagePathEnd]);
            }
            ...
            if (req.files && req.files['sound']) {
                ...
                await Insert.UpdateAlertBoxSound([userId, "Bits", soundPathEnd]);
            }
            res.redirect("/bits");
        } catch (error) {
...
BitBoxRoute.post('/test', async (req, res) => {
    const key = req.signedCookies.access_validator;
    if (!key) {
        return res.redirect("/?index=true");
    };
    ...
    const sessionData = JSON.parse(await client.get(`sess:${key}`));
    Alerts.testAlert(sessionData.userId, "Bits", sessionData.username)
    res.redirect("/bits")
})