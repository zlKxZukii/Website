...
export let SubGiftBoxRoute = express.Router()

SubGiftBoxRoute.get("/", async (req, res) => {
    ...
    try {
        ...
        const obj = {
            ...
            boxes: "Subscriber Geschenke",
            title: "Subscriber Geschenke Box",
            showBody: true,
        ...
        if (!user) {
            for (const element of AlertBox) {
                if (element.type === "Subscriber Geschenke") {
                    Object.assign(obj, {
                        [obj.boxes]: {
        ...
        res.render("main/boxes/subGiftBox.ejs", obj);
    } catch (error) {
        console.log(error);
...
SubGiftBoxRoute.get("/:key/renew", async (req, res) => {
    const key = req.signedCookies.access_validator;
    if (!key) {
        return res.redirect("/?index=true");
    };
...
SubGiftBoxRoute.post('/save', async (req, res) => {
    ...
    try {
        ...
        const { state, volume, duration, responseText, color, selectedLayout, viewer, streamer, amount, tier, cumulativ, family, size, decoration, weight } = req.body;

        if (user) {
            const alertBox = user.alertBox['Subscriber Geschenke'].settings;
        ...
        await Insert.UpdateAlertBoxSettings([sessionData.userId, "Subscriber Geschenke", JSON.stringify({ state, volume, duration, responseText, color, layout: selectedLayout, viewer, streamer, amount, tier, family, size, decoration, weight })]);
    } catch (error) {
...
SubGiftBoxRoute.post('/upload/raid',
    ...
    async (req, res) => {
        ...
        try {
            ...
            const userFolder = path.join("uploads", "subgifts", String(userId));
...
SubGiftBoxRoute.post('/test', async (req, res) => {
    const key = req.signedCookies.access_validator;
    if (!key) {
        return res.redirect("/?index=true");
    };
    ...
    const sessionData = JSON.parse(await client.get(`sess:${key}`));
    Alerts.testAlert(sessionData.userId, "Subscriber Geschenke", sessionData.username)
    res.redirect("/subgifts")
})